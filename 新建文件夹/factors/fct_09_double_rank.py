...
def calc_double_rank(
    df_item: pd.DataFrame, w1: int = 10, w2: int = 1, m1: int = 0, m2: int = 0
):
    if m2 == 1:
        df_res = df_item.pct_change(1)
    else:
        df_res = df_item
    if m1 == 1:
        ...
        df_res = df_item
    df_res = ops.calc_ts_rank(df_res, w1)
    if w2 > 1:
...
def calc_feat(
    ...
    df_res = calc_double_rank(df_item=df_item, w1=w1, w2=w2, m1=m1, m2=m2)
