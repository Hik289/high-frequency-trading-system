...
def calc_by_val_two(
    ...
    def _calc_sng_row(_idx):
        ...
        top_bound = -np.sort(-part_by, axis=0)[top_idx, list(range(part_by.shape[1]))]
    ...
    res_top = np.full(ar_val.shape, np.nan)
    res_bot = np.full(ar_val.shape, np.nan)
    for i in range(res_top.shape[0]):
        tmp_res = _calc_sng_row(i)
        res_top[i] = tmp_res[0]
        res_bot[i] = tmp_res[1]
    df_top = pd.DataFrame(res_top, index=df_val.index, columns=df_val.columns)
    df_bot = pd.DataFrame(res_bot, index=df_val.index, columns=df_val.columns)
    if m1 == 0:
        with np.errstate(divide="ignore", invalid="ignore"):
            df_fct = ops.calc_diff(df_top, df_bot)
    else:
        with np.errstate(divide="ignore", invalid="ignore"):
            df_fct = df_top / df_bot
...
params = list(
    itertools.product(
        val_lst, by_lst, wr_lst, ws_lst, q_lst, m1_lst, m2_lst, deform_lst
    )
)
